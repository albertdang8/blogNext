          ...
          <li key={post.id} className="flex flex-col items-center">
            <div className="">
              <Image
                ...
                width={250}
                height={250}
                className="h-[200px] rounded-sm mb-4 object-cover"
              />
            </div>
            <div>
              <h2 className="max-w-[250px] font-bold capitalize text-2xl pb-8 border-b">
                {post.title}
              </h2>
              <h4 className="pt-2">
                Tags:{" "}
                {post.tags.map((tag: string, index: number) => (
                  <span key={index}>
                    <span className="cursor-pointer hover:text-accentLight transition-colors">
                      #{tag}
                    </span>
